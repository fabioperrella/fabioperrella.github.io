...
But, when I opened the PR, my team warned me that maybe we should use a [connection pool](https://github.com/mperham/connection_pool) instead of using a singleton because we were not sure if the AWS gems were thread-safe.
...
However, as we were not 100% if it would introduce a new bug, we draw this plan:

1. Change `wt_s3_signer` gem to accept `Aws::S3::Client` by dependency injection optionally. With that, it was possible to test the new approach in our application and not break gem's compatibility

2. Update `wt_s3_signer` in Storm and pass a singleton instance of `Aws::S3::Client` to `wt_s3_signer`

3. After we were certain that no problems happended, change `wt_s3_signer` to create internally, by default, a singleton instance of `Aws::S3::Client`

## First results

After doing steps 1 and 2, we come back to the graph on Appsignal, expecting that the number of requests would be close to zero, but then it reduced only a part of it:

![image4](../debug-aws-assets/image4.png)

So, I thought that could be more places instatiating AWS clients.

Then, I repeated the same strategy to find other places and I found 2 more:
...
For `sqewer` we used the same 3-steps strategy that we defined for `wt_s3_signer`.

## Impressive result!

Finally, after fixing these 3 places, the number of requests decreased almost to zero!
