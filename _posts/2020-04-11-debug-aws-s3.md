# How we boosted response time of a service 2.5x using a singleton
...
The problem started when we noticed on Appsginal, some HTTP requests to the AWS metadata endpoint taking too long to complete:
...
If I had seen this issue alone, I would have taken much more time to find the origin of these requests because
as we can see, Appsignal does not show this information. By the way, we asked them for this feature!

Thanks to the engineers experienced with AWS at WeTransfer, I saved some time with that, and I learned that the AWS gems use this endpoint to get credentials and interact with AWS services.
...
My goal was to find a test scenario that I could run and debug it locally.

Unfortunately, when I ran the tests, no one reached this point.

Note1: I used RSpec's option `--fail-fast` to abort the execution in the first failure.
...
Then, I kept going up in the stack trace, trying to find a point where I could start debugging it on my laptop.

Fortunately, I found a point in the next iteraction, in the method [providers](https://github.com/aws/aws-sdk-ruby/blob/7c730a81508981ec379920e04af60776fef69386/gems/aws-sdk-core/lib/aws-sdk-core/credential_provider_chain.rb#L21):
...
I'm a fan of the [pry-byebug](https://github.com/deivid-rodriguez/pry-byebug) gem beucase it's possible to use commands such as `up`, `down`, `step`, `next` and `finish` to debug a code. So, I used it. I strongly reccomend learning these commands.
...
So, I could understand that this method `resolve` checked which credential provider was configured in the current environment, and for the `test` environment, it was using the `AssumeRoleCredentials` one.
...
After learning that, I asked myself why can't we use static credentials in production, by setting those environment variables, as we use in the test environment.

If we used that, those HTTP requests would not have being done and I could finish the investigation.

Then my teammates explained why we can't do it, which is also described on [AWS docs](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/iam-roles-for-amazon-ec2.html):
...
Note: Thanks [iTerm2](https://iterm2.com/) for having this text search with highlight! I was a Linux user in the past, and there was no such feature on the consoles there.
...
We expected the AWS gems to cache credentials in some way, because they expire only after some time.
...
When it's not necessary to refresh it, it only returns `@credential`, which is cached in the instance.

So this confirmed my theory! If we instantiate new client objects, there will be no cached `@credential` and it will fetch a fresh one.

Then, I started searching on [AWS docs](https://docs.aws.amazon.com/sdk-for-ruby/v3/developer-guide/setup-config.html) if they mention something about that. It would have helped me a lot if I had found something like this:

> We recommend using a singleton instance for the clients otherwise, credentials will not be cached/reused and your application will spend more time getting fresh credentials every time.

# How to prove my theory

To prove that my theory was right, I wanted to execute those methods and confirm that the HTTP requests were made only for fresh client instances.

It was not possible to reproduce it in my laptop, because as I showed before, it uses a different credentials provider.

Then, my only option was to debug on the staging server, and there were no `pry-byebug` available on this environement.

That's when Ruby metaprogramming helped me!
...
I pasted that into the staging Storm's console and got the following result:
...
At this point, my plan was to change the `wt_s3_signer` gem to not create an instance of `Aws::S3::Client` on each call to [WT::S3Signer.for_s3_bucket](https://github.com/WeTransfer/wt_s3_signer/blob/v0.1.0/lib/wt_s3_signer.rb#L31).

But, when I opened the PR, my team warned me that maybe we should use a [connection pool](https://github.com/mperham/connection_pool) instead of using a singleton because we were not sure that the AWS gems were thread-safe.
...
However, as we were not 100% confident about the thread-safety, we draw the following plan:

1. Change `wt_s3_signer` gem to accept `Aws::S3::Client` by dependency injection optionally. With that, it would be possible to test the new approach in Storm without breaking gem's compatibility

2. Update `wt_s3_signer` in Storm and inject the singleton instance of `Aws::S3::Client` into `wt_s3_signer`

3. After we are sure that no problems happended, change `wt_s3_signer` to create the singleton `Aws::S3::Client` internally, by default

## First results

After doing steps 1 and 2, we looked again at the graph that shows the number of HTTP requests to the AWS endpoint on Appsignal, hoping it would be close to zero, but reduced only a part of it:

![image4](../debug-aws-assets/image4.png)

So, I thought that could be more places instatiating AWS clients.

Then, I repeated the same strategy to find other places and I found two others:
...
In this case, the method `Aws::S3::Bucket.new` [accepts a client as an argument](https://github.com/aws/aws-sdk-ruby/blob/0de23463adbca4d3fce234107fa5fc9a70e49e54/gems/aws-sdk-s3/lib/aws-sdk-s3/bucket.rb#L26), but the default is to create a new instance.
...
I also found [this line](https://github.com/WeTransfer/sqewer/blob/49a85115b160781a545a8b482001179475dfe37c/lib/sqewer/connection.rb#L58) inside the `sqewer` gem:
...
Finally, after fixing those 3 places, the number of requests dropped almost to zero!

![image5](../debug-aws-assets/image5.png)

However, the most impressive result was the performance boost the we gained which was not expected:

![image6](../debug-aws-assets/image6.png)

Almost 2.5 times faster!!

## Conclusion

Ruby gems help us a lot but we need to use them properly.

Fortunately, when we don't understant what is happenning, it's possible to debug them, either by using `binding.pry` of metaprogramming.

Don't be afraid to try that, we can learn a lot by digging into these codes!
