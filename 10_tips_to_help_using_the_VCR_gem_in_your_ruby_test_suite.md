...
Depending on the order the tests run, theses values will be different and because of it, it can break some scenarios with VCR because the payload or the query string of a request will be different comparing with the recorded one.
...
For instance, if testing an API which will delete a resource in the server:
...
In the first time we run the scenario, the cassette will be recorded and it will work.

But if we change something and try to record the cassette again, the resource with id 40 will not exist anymore and the execution will fail!
...
A good practice is to run the tests in random order to force the scenarios to be independent of the others.

Using Rspec, it is possible to do it with the following configuration:
...
Running the tests in random order, if the test with the second cassette runs first, it will break because it will try to do a request to create the token which is not recorded.
...
To avoid this noise in the diffs, it is possible to use the file `.gitattributes` to hide them. The majority of the git hosts (such as github, gitlab) can recognize it:
...
**Attention**: the diff will be hidden in all git clients too, for example, `tig` or `gitx`, but it is possible to remove this file temporarily to see the diff in the local environment, if necessary.

## Wraping up

I hope these tips and tricks can help the devs using tests with VCR.
